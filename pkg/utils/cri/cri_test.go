...
const crlctlBin = "/usr/bin/crictl"

func TestNewContainerRuntime(t *testing.T) {
	execLookPathOK := fakeexec.FakeExec{
		LookPathFunc: func(cmd string) (string, error) { return crlctlBin, nil },
	}
	...
	cases := []struct {
		...
		{"valid: default cri socket", execLookPathOK, DefaultDockerCRISocket, true, false},
...
func TestIsRunning(t *testing.T) {
	...
	cases := []struct {
		...
		{"valid: docker is running", DefaultDockerCRISocket, dockerExecer, false},
		{"invalid: docker is not running", DefaultDockerCRISocket, dockerExecer, true},
	}
...
func TestListKubeContainers(t *testing.T) {
	...
	cases := []struct {
		...
		{"valid: list containers using docker", DefaultDockerCRISocket, false},
	}
...
func TestRemoveContainers(t *testing.T) {
	...
	cases := []struct {
		...
		{"valid: remove containers using docker", DefaultDockerCRISocket, []string{"k8s_c1", "k8s_c2", "k8s_c3"}, false},
		{"invalid: docker rm failure", DefaultDockerCRISocket, []string{"k8s_c1", "k8s_c2", "k8s_c3"}, true},
		{"invalid: docker stop failure", DefaultDockerCRISocket, []string{"k8s_c1", "k8s_c2", "k8s_c3"}, true},
	}
...
func TestPullImage(t *testing.T) {
	...
	cases := []struct {
		...
		{"valid: pull image using docker", DefaultDockerCRISocket, "image1", false},
		{"invalid: docker pull error", DefaultDockerCRISocket, "image2", true},
	}
...
func TestImageExists(t *testing.T) {
	...
	cases := []struct {
		...
		{"valid: test if image exists using docker", DefaultDockerCRISocket, "image1", false},
		{"invalid: docker inspect failure", DefaultDockerCRISocket, "image2", true},
	}
...
func TestDetectCRISocketImpl(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			existingSockets: []string{},
			expectedError:   false,
			expectedSocket:  DefaultDockerCRISocket,
		},
		{
