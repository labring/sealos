...
func printCommands(name string, env []string, info buildah.BuilderInfo) {
	...
	for i := range info.OCIv1.Config.Entrypoint {
		cmds = append(cmds, stringsutil.FormalizeWorkingCommand(name, info.Container, typeKey, expansion.Expand(info.OCIv1.Config.Entrypoint[i], mapping)))
	}

	for i := range info.OCIv1.Config.Cmd {
		cmds = append(cmds, stringsutil.FormalizeWorkingCommand(name, info.Container, typeKey, expansion.Expand(info.OCIv1.Config.Cmd[i], mapping)))
	}
