...
func newStaticPodCmd() *cobra.Command {
	...
	staticPodCmd.PersistentFlags().
		StringVar(&staticPodPath, "path", "/etc/kubernetes/manifests", "default kubernetes static pod path")
	return staticPodCmd
}
...
func newLvscareCmd() *cobra.Command {
	...
	lvscareCmd := &cobra.Command{
		...
		PreRunE: func(cmd *cobra.Command, args []string) error {
			if len(obj.master) == 0 && !setImage {
				return errors.New("master not allow empty")
			}
			return nil
		},
	...
	lvscareCmd.Flags().
		StringVar(&obj.name, "name", constants.LvsCareStaticPodName, "generator lvscare static pod name")
	lvscareCmd.Flags().
		StringVar(&obj.image, "image", v1beta1.DefaultLvsCareImage, "generator lvscare static pod image")
...
func genNewPod(obj lvscarePod) error {
	...
	if err = file.MkDirs(staticPodPath); err != nil {
		return fmt.Errorf("init dir is error: %w", err)
	}
	err = os.WriteFile(path.Join(staticPodPath, fileName), []byte(yaml), 0o755)
	if err != nil {
		return err
	}
...
func setNewPodImage(obj lvscarePod) error {
	...
	if file.IsExist(podPath) {
		...
		err = os.WriteFile(path.Join(staticPodPath, fileName), data, 0o755)
		if err != nil {
			return err
		}
