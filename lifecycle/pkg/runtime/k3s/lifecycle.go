...
func (k *K3s) deleteNode(node string) error {
	...
	if len(masterIPs) > 0 {
		...
		if err := k.removeNode(node); err != nil {
			logger.Warn(fmt.Errorf("delete nodes %s failed %w", node, err))
		}
	}
	return nil
}
...
func (k *K3s) removeNode(ip string) error {
	...
	nodeName, err := k.execer.CmdToString(
		k.cluster.GetMaster0IPAndPort(),
		fmt.Sprintf(
			"kubectl get nodes -o wide | awk '$6==\"%s\" {print $1}'",
	...
	return k.execer.CmdAsync(
		k.cluster.GetMaster0IPAndPort(),
		fmt.Sprintf("kubectl delete node %s --ignore-not-found=true", nodeName),
	)
}
