...
func (k *KubeadmConfig) FinalizeFeatureGatesConfiguration() {
	...
	for i, args := range extraArgs {
		...
		for j, v := range args {
			if v.Name == "feature-gates" && v.Value != "" {
				if newValue, ok := updateFeatureGatesConfiguration(v.Value, k.KubernetesVersion).(string); ok {
	...
	if gates, ok := updateFeatureGatesConfiguration(
		k.KubeletConfiguration.FeatureGates, k.KubernetesVersion).(map[string]bool); ok {
		k.KubeletConfiguration.FeatureGates = gates
	}
}
...
func LoadKubeadmConfigs(
	...
	if err != nil {
		...
		if initCfg, ok := initConfig.(*kubeadm.InitConfiguration); ok {
			kubeadmConfig.InitConfiguration = *initCfg
		}
	}
	...
	if err != nil {
		...
		if clusterCfg, ok := clusterConfig.(*kubeadm.ClusterConfiguration); ok {
			kubeadmConfig.ClusterConfiguration = *clusterCfg
		}
	}
	...
	if err != nil {
		...
		if proxyCfg, ok := kubeProxyConfig.(*kubeproxyconfigv1alpha1.KubeProxyConfiguration); ok {
			kubeadmConfig.KubeProxyConfiguration = *proxyCfg
		}
	}
	...
	if err != nil {
		...
		if kubeletCfg, ok := kubeletConfig.(*kubeletconfigv1beta1.KubeletConfiguration); ok {
			kubeadmConfig.KubeletConfiguration = *kubeletCfg
		}
	}
	...
	if err != nil {
		...
		if joinCfg, ok := joinConfig.(*kubeadm.JoinConfiguration); ok {
			kubeadmConfig.JoinConfiguration = *joinCfg
		}
	}
