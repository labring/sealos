...
func GetHostIPAndPortOrDefault(host, _default string) (string, string) {
	if !strings.ContainsRune(host, ':') {
...
func GetHostIPAndPortSlice(hosts []string, _default string) (res []string) {
	for _, ip := range hosts {
		_ip, port := GetHostIPAndPortOrDefault(ip, _default)
