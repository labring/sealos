...
var _ = Describe("E2E_sealos_images_buildrun_test", func() {
	fakeClient := operators.NewFakeClient("")
	Context("sealos images build and run suit", func() {
		It("images build image running cluster", func() {
			...
			err = fakeClient.Image.BuildImage(
				"test-build-image:rootfs-sealctl",
				tmpdir,
			...
			utils.CheckErr(
				err,
				fmt.Sprintf(
					"failed to pull image docker.io/altinity/clickhouse-operator:0.18.4: %v",
					err,
				),
			)
			...
			utils.CheckErr(
				err,
				fmt.Sprintf(
					"failed to validate image sealos.hub:5000/altinity/clickhouse-operator:0.18.4: %v",
					err,
				),
			)
		})
