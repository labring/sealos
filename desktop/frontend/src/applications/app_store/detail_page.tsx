...
const DetailPage = ({ app }: { app: TApp }) => {
	const { installedApps, installApp, openApp } = useAppStore(({ installedApps, installApp, openApp }) => ({ installedApps, installApp, openApp }))
	...
	const changeVersion = async (v: string) => {
		if (v !== currentVersionInfo?.version) {
			...
			// mock change 
			if (
				appDetail?.currentVersionInfo.version &&
				v !== appDetail?.currentVersionInfo.version
			) {
			...
			<div className={clsx(styles.growcont, " flex flex-row ")}>
				<div className={clsx(styles.detailinfo, 'pl-8 pr-10')}>
					<div className={clsx(styles.infobrief, "flex flex-row flex-wrap")}>
						...
						<div className='border ml-5 bg-white px-5 py-2 mt-8'>{currentVersionInfo?.supportProcessor}</div>
						<div className='border ml-5 bg-white px-5 py-2 mt-8'>{currentVersionInfo?.memory}</div>
					</div>

					<div aria-expanded={`${open}`} ref={setTarget} className={clsx(styles.codebg, "inline-flex flex-col pl-5 py-2 pr-72 my-12 cursor-pointer")} onClick={() => copyContent()}>
						{
							installGuide?.map(item => (<div className='text-white' key={item}> {item}</div>))
						}
					</div>
					<Popover appearance="inverted" positioning={{ target }} open={open} onOpenChange={onOpenChange}>
						<PopoverSurface >
							copied!
						</PopoverSurface>
					</Popover>
				...
				<div className={clsx(styles.detailversions, "flex flex-col mt-8")}>
					{versions.map(item => (<Button onClick={() => changeVersion(item)} appearance={item === currentVersionInfo?.version ? 'primary' : undefined} key={item}>{item}</Button>))}
				</div>
			</div>
		</div >
	)
