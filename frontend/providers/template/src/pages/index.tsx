...
export default function AppList() {
  ...
  const { data: FastDeployTemplates, refetch } = useQuery(['listTemplte'], getTemplates, {
    refetchInterval: 5 * 60 * 1000,
    staleTime: 5 * 60 * 1000
  });

  const filterData = useMemo(() => {
    const typeFilteredResults = FastDeployTemplates?.filter((item: TemplateType) => {
  ...
  }, [FastDeployTemplates, appType, searchValue]);
          ...
          {filterData?.map((item: TemplateType) => {
            return (
              ...
              >
                ...
                <Flex mt="auto" justifyContent={'space-between'} alignItems={'center'} gap={'20px'}>
