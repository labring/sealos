...
function ensureAnnotations(meta: Required<AnyK8sObject>['metadata']) {
...
function parseYamlList(yamlList: string[]): AnyK8sObject[] {
  ...
  for (const str of yamlList) {
    const docs = yaml.loadAll(str).filter(Boolean) as AnyK8sObject[];
    all.push(...docs);
  }
  return all;
}
 ...
 * - create/replace:
 *   1) find Instance doc, ensure annotation ready
...
export async function applyWithInstanceOwnerReferences(
  ...
  const instanceResources = await deps.applyYamlList([yaml.dump(instance)], mode);
  ...
  const ownerReferences = generateInstanceOwnerReference(instanceName, uid);
  const instanceOwnerRef = ownerReferences[0];
  ...
  if (dependentResources.length > 0) {
    const dependentYamlList = dependentResources.map((r) => yaml.dump(r));
