...
export const handleK8sError = (err: any): Partial<ApiResponse> => {
  // Prioritize err.body for Kubernetes API errors
  ...
  return {
    code: errorData?.code || ResponseCode.SERVER_ERROR,
    message: errorData?.message || ResponseMessages[ResponseCode.SERVER_ERROR]
  };
};
