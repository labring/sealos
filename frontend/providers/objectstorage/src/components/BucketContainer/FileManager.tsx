...
import { searchText, useCopyData } from '@/utils/tools';
...
export default function FileManager({ ...styles }: FlexProps) {
  ...
  return (
    <Flex direction={'column'} {...styles}>
      <HStack w="full" my="16px">
        <PathLink />
        <InputGroup mb="25px" variant={'secondary'} alignItems={'center'} size={'sm'}>
          <InputLeftElement>
      ...
      <HStack justifyContent={'flex-end'} minH={'max-content'} mt="auto" mb="0">
        ...
        <ButtonGroup variant={'white-bg-icon'}>
          {
            // 当 stack 是 0，代表没有更多的object
            // 当 stack 是 1，代表有更多的object 但是在第一页或第二页
          }
          <Button
            isDisabled={
              pageStack.length === 0 ||
