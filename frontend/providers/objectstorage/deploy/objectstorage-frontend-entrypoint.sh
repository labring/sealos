...
RELEASE_NAME=${RELEASE_NAME:-"objectstorage-frontend"}
RELEASE_NAMESPACE=${RELEASE_NAMESPACE:-"objectstorage-frontend"}
SERVICE_NAME="objectstorage-frontend"
...
if ! helm status "${RELEASE_NAME}" -n "${RELEASE_NAMESPACE}" >/dev/null 2>&1; then
  adopt_namespaced_resource "${RELEASE_NAMESPACE}" secret object-storage-frontend-secret
...
config_jwt_internal=$(get_cm_value jwtInternal)

add_set_string objectstorageConfig.cloudDomain "${config_cloud_domain:-${SEALOS_CLOUD_DOMAIN:-${cloudDomain:-}}}"
add_set_string objectstorageConfig.cloudPort "${config_cloud_port:-${SEALOS_CLOUD_PORT:-${cloudPort:-}}}"
add_set_string objectstorageConfig.httpPort "${config_http_port:-${SEALOS_HTTP_PORT:-${httpPort:-}}}"
add_set_string objectstorageConfig.disableHttps "${config_disable_https:-${SEALOS_DISABLE_HTTPS:-${disableHttps:-}}}"
add_set_string objectstorageConfig.certSecretName "${config_cert_secret_name:-${SEALOS_CERT_SECRET_NAME:-${certSecretName:-}}}"
add_set_string objectstorageConfig.monitorUrl "${MONITOR_URL:-${monitorUrl:-}}"
add_set_string objectstorageConfig.appLaunchpadUrl "${APP_LAUNCHPAD_URL:-${appLaunchpadUrl:-}}"
add_set_string objectstorageConfig.accountServer "${SEALOS_ACCOUNT_SERVER:-${accountServer:-}}"
add_set_string objectstorageConfig.accountServerTokenJwtKey "${config_jwt_internal:-${SEALOS_ACCOUNT_SERVER_TOKEN_JWT_KEY:-${accountServerTokenJwtKey:-}}}"
add_set_string objectstorageConfig.appTokenJwtKey "${config_jwt_internal:-${SEALOS_APP_TOKEN_JWT_KEY:-${appTokenJwtKey:-}}}"
