...
Managed public-domain availability is checked in two stages. The form first runs local prefix validation, reserved-prefix validation, and same-form managed host duplicate checks, then `/api/platform/checkPublicDomain` uses the user's kubeconfig to list same-namespace Ingresses before performing a server-side dry-run Ingress create. Same-workspace conflicts return a safe `conflictOwner` attribution for Launchpad apps, Devbox-style recommended labels, Helm-style components, or an unlabeled Ingress fallback. The prefix editor triggers local duplicate feedback and then that availability check shortly after the user changes a managed public-domain prefix; the submit flow repeats both checks as a final pre-apply guard. The dry-run reaches `vingress.sealos.io`, so cross-namespace host ownership conflicts can be shown before the user applies the app without exposing the other workspace's resource details. Create/update/apply paths still keep the admission-webhook error translation as the final fallback.
