...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  try {
    ...
    const queryType: { [key: string]: string } = {
      [DBTypeEnum.mysql]: `sum(mysql_global_variables_innodb_buffer_pool_size{$, workloads_kubeblocks_io_instance="${dbName}-${DBTypeEnum.mysql}"}) by (namespace,app_kubernetes_io_instance,pod)`,
      [DBTypeEnum.notapemysql]: `sum(mysql_global_variables_innodb_buffer_pool_size{$, workloads_kubeblocks_io_instance="${dbName}-${DBTypeEnum.notapemysql}"}) by (namespace,app_kubernetes_io_instance,pod)`
    };
