...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  try {
    ...
    const queryType: { [key: string]: string } = {
      [DBTypeEnum.postgresql]: `rate(pg_stat_database_blk_write_time_total{$, workloads_kubeblocks_io_instance="${dbName}-${DBTypeEnum.postgresql}"}[1m])`
    };
    ...
    const adapterDataCharts = (data: MonitorDBResult) => {
      ...
      const desiredResult = temp.map((dataObject, index) => {
        ...
        // y value
        const data = dataObject.values.map((item) => (parseFloat(item[1]) * 1000).toFixed(3));
