...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  try {
    ...
    const queryType: { [key: string]: string } = {
      'apecloud-mysql': `sum(rate(mysql_global_status_table_locks_immediate{$, app_kubernetes_io_instance="${dbName}"}[1m])) by (namespace,app_kubernetes_io_instance,pod)`,
      mysql: `sum(rate(mysql_global_status_table_locks_immediate{$, app_kubernetes_io_instance="${dbName}"}[1m])) by (namespace,app_kubernetes_io_instance,pod)`
    };
