...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  try {
    ...
    const queryType: { [key: string]: string } = {
      [DBTypeEnum.mysql]: `sum(rate(mysql_global_status_aborted_connects{$, workloads_kubeblocks_io_instance="${dbName}-${DBTypeEnum.mysql}"}[1m])) by (namespace,app_kubernetes_io_instance,pod)`,
      [DBTypeEnum.notapemysql]: `sum(rate(mysql_global_status_aborted_connects{$, workloads_kubeblocks_io_instance="${dbName}-${DBTypeEnum.notapemysql}"}[1m])) by (namespace,app_kubernetes_io_instance,pod)`
    };
