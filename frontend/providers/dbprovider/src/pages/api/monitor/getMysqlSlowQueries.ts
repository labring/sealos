...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  try {
    ...
    const queryType: { [key: string]: string } = {
      [DBTypeEnum.mysql]: `sum(rate(mysql_global_status_slow_queries{$,workloads_kubeblocks_io_instance="${dbName}-mysql"}[1m])) by (namespace,app_kubernetes_io_instance,pod)`,
      [DBTypeEnum.notapemysql]: `sum(rate(mysql_global_status_slow_queries{$,workloads_kubeblocks_io_instance="${dbName}-mysql"}[1m])) by (namespace,app_kubernetes_io_instance,pod)`
    };
