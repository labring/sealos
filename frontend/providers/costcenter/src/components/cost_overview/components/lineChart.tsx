...
import { subSeconds, addDays, differenceInDays, subDays, formatISO, parseISO, format, isSameDay, startOfDay } from 'date-fns';
  ...
  const source = [...INITAL_SOURCE,
    ...(data?.data?.status.item
      ...
      .reduce<[Date, number, number, number, number][]>((pre, cur) => {
        console.log('time', cur[0])
        if (pre.length !== 0) {
          const precost = pre[pre.length - 1];
          if (isSameDay(precost[0], cur[0])) {
            cur
              .forEach((v, i) => i > 0 && ((precost[i] as number) += v as number))
            return pre;
          }
        }
