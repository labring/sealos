...
export class Ingress extends KubeObject<NamespaceScopedMetadata, IngressStatus, IngressSpec> {
  ...
  getServiceNamePort(): ExtensionsBackend | undefined {
    ...
    const servicePort =
      defaultBackend?.service?.port.number ?? defaultBackend?.service?.port.name ?? backend?.servicePort;
  ...
  getLoadBalancers() {
    return this.status?.loadBalancer?.ingress?.map((address) => address.hostname || address.ip) ?? [];
  }
}
