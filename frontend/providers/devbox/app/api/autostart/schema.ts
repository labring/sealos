...
const AutostartRequestSchema = z
  .object({
  ...
  .openapi({
    description: 'Request body for autostart configuration'
  });

const AutostartSuccessResponseSchema = z
  .object({
    data: z.object({
      devboxName: z.string().openapi({
        description: 'Devbox name'
...
const AutostartErrorResponseSchema = z
  .object({
...
export { AutostartRequestSchema, AutostartSuccessResponseSchema, AutostartErrorResponseSchema };
