  ...
  try {
    ...
    if (action === 'stop' || action === 'shutdown') {
      ...
      await k8sCustomObjects.patchNamespacedCustomObject(
        ...
        'devboxes',
        devboxName,
        { spec: { state: action === 'stop' ? 'Stopped' : 'Shutdown' } },
        undefined,
        undefined,
        undefined,
        {
      ...
      return jsonRes({ data: 'success stop devbox' });
    }
    ...
    if (action === 'restart') {
      ...
      return jsonRes({ data: 'success restart devbox' });
    }

    return jsonRes({ code: 400, message: 'Unknown action' });
  } catch (err: any) {
    return jsonRes({
      code: 500,
