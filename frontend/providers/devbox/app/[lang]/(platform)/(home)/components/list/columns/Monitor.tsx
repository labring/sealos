...
export const Monitor = memo<MonitorProps>(
  ({ row, type }) => {
    ...
    return <MonitorChart type={chartType} data={data || EMPTY_MONITOR_DATA} className="h-9 w-44" />;
  },
  (prev, next) => {
    const prevData = prev.type === 'cpu' ? prev.row.original.usedCpu : prev.row.original.usedMemory;
    const nextData = next.type === 'cpu' ? next.row.original.usedCpu : next.row.original.usedMemory;
