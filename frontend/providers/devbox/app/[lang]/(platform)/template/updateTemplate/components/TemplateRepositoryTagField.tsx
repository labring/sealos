...
const TagList = ({ tags, title }: { tags: Tag[], title: string }) => {
  ...
  return <Box>
    <Text fontSize="12px" fontWeight={500} color={'grayModern.600'} mb={'4px'}>{title}</Text>
...
export default function TemplateRepositoryTagField() {
  ...
  let tagListCollection = tagList.reduce((acc, tag) => {
    if (!acc[tag.type]) {
      acc[tag.type] = [];
    }
  ...
  return <Flex align={'center'}>
    <MyFormLabel isRequired width="108px" m='0' fontSize="14px" alignSelf={'flex-start'}>{t('tags')}</MyFormLabel>
