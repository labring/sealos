...
const EditTemplateModal: FC<CreateTemplateModalProps> = ({
  ...
  return (<>
        ...
        <ModalBody py={'32px'} px={'56px'} >
          ...
          <Flex flexDirection={'column'} h={'full'}>
            ...
            <Flex h={'364px'} flex={'auto'} overflow={'auto'} flexDirection={'column'}>
              <MyTable needRadius columns={columns} data={templateList} gridTemplateColumns={'120px 180px 180px auto'} />
              {templateList.length === 0 && <Flex
    ...
    {!!deletedTemplateVersion && <DeleteTemplateVersionModal
      isLasted={templateList.length <= 1}
      version={deletedTemplateVersion.name}
      template={templateRepositoryName}
      isOpen={DeleteTemplateVersionHandle.isOpen} onClose={DeleteTemplateVersionHandle.onClose}
