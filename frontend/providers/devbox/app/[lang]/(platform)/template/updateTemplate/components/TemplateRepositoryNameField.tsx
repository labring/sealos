...
export default function TemplateRepositoryNameField({ isDisabled = false }: { isDisabled?: boolean }) {
  const ctx = useFormContext<{ name: string }>();
  const t = useTranslations()
  return <Flex justify={'space-between'} align={'center'}>
    <MyFormLabel isRequired width="108px" m='0' fontSize="14px">{t('name')}</MyFormLabel>
    <Input
      {...ctx.register('name')}
