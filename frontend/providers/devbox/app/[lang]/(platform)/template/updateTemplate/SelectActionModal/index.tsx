...
const SelectTemplateModal = ({
  ...
  isOpen: boolean;
  onClose: () => void;
  templateRepositoryList: { iconId: string | null, name: string, description: null | string, uid: string }[]
  ...
  const [selectedTemplateRepoUid, setSelectedTemplateRepoUid] = useState(templateRepositoryList?.[0]?.uid);
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
    ...
    >
      ...
      <ModalContent maxW="504px" margin={'auto'}>
        ...
        <ModalBody pt={'32px'} px={'52px'} pb={'24px'}>
          <ModalCloseButton />
          <Text mb={'10px'} color={'grayModern.900'} fontWeight={500}>{t("select_template_tips")}</Text>
          <TemplateDropdown
        ...
        <ModalFooter gap={'16px'} px={'52px'}>
          <Button variant={'unstyled'} color={'brightBlue.600'} display={'flex'} onClick={() => {
            onOpenCreate()
            onClose()
          }}>
