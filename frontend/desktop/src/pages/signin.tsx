...
function usePassword({ setError }: { setError: React.Dispatch<React.SetStateAction<string>> }) {
  ...
  const verify = () => {
    if (!strongUsername(username.current)) {
      setError(
        t('username tips') || 'Username must be 3-16 characters, including letters, numbers'
      );
      return false;
    }
  ...
  const sign = () =>
    request.post<any, ApiResp<Session>>('/api/auth/password', {
...
function useSms({ setError }: { setError: React.Dispatch<React.SetStateAction<string>> }) {
  ...
  const sign = () =>
    request.post<any, ApiResp<Session>>('/api/auth/phone/verify', {
      phoneNumbers: phoneNumber.current,
      code: checkValue.current
    });
    ...
    return (
      <>
        ...
        <InputGroup variant={'unstyled'} bg="rgba(255, 255, 255, 0.65)">
          ...
          />
          <InputRightAddon>{checkIsVaild() && <Img src={updateIcon.src}></Img>}</InputRightAddon>
        </InputGroup>
      </>
    );
  };
...
export default function Login({
  ...
  const verify = () =>
    ((tabIndex === 0 && smsVerify()) || (tabIndex === 1 && passwordVerify())) && verifyAgree();
  const getSign: FormEventHandler = async (e) => {
    ...
    if (tabIndex === 0) {
      ...
      if (isFirst) {
        ...
        setIsLoading(true);
        try {
          const { data } = await request.post<any, ApiResp<Session>>('/api/auth/password', {
            user: username.current,
            password: password.current
          });
          setSession(data!);
