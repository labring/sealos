  ...
  const SmsModal = () => {
    ...
    return <><InputGroup variant={'unstyled'} bg='rgba(255, 255, 255, 0.65)' mt={'20px'}>
      ...
      <InputGroup variant={'unstyled'} bg='rgba(255, 255, 255, 0.65)'>
        ...
        <Input type='password' placeholder={t('verify code tips') || ''} pl={'12px'}
          variant={'unstyled'}
          id="verifyCode"
          fontSize='14px'
          fontWeight='400'
  ...
  const getSign: FormEventHandler = async (e) => {
    ...
    if (tabIndex === 0) {
      ...
      if (isFirst) {
        ...
        setIsLoading(true)
        try {
          const { data } = await request.post<any, ApiResp<Session>>('/api/auth/password', { username: username.current, password: password.current })
          setSession(data!)
          updateUser()
          router.replace('/')
        } catch {
