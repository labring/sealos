...
function deploy_minio() {
  ...
  sed -i 's/{CONSOLE_ACCESS_KEY}/$CONSOLE_ACCESS_KEY/g' manifests/minio/deploy.yaml.tmpl
  sed -i 's/{CONSOLE_SECRET_KEY}/$CONSOLE_SECRET_KEY/g' manifests/minio/deploy.yaml.tmpl
