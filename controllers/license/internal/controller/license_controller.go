...
func (r *LicenseReconciler) reconcile(
	...
	case licensev1.ValidationClusterIDMismatch:
		...
		license.Status.Reason = fmt.Sprintf(
			"cluster id mismatch, license cluster id: %s, cluster id: %s",
			claims.ClusterID,
			r.ClusterID,
		)
		r.Logger.V(1).
			Info("cluster id mismatch", "license", license.Namespace+"/"+license.Name, "cluster id", r.ClusterID, "license cluster id", claims.ClusterID)
	...
	case licensev1.ValidationClusterInfoMismatch:
		...
		license.Status.ValidationCode = valid
		license.Status.Reason = fmt.Sprintf(
			"cluster info mismatch, license cluster info: %v",
			claims.Data,
		)
		r.Logger.V(1).
			Info("cluster info mismatch", "license", license.Namespace+"/"+license.Name, "cluster info", claims.Data)
		_ = r.Status().Update(ctx, license)
		return requeueRes, nil
	case licensev1.ValidationExpired:
		...
		r.Logger.V(1).
			Info("license is invalid", "license", license.Namespace+"/"+license.Name, "reason", valid)
		_ = r.Status().Update(ctx, license)
		return requeueRes, nil
	case licensev1.ValidationError:
		...
		r.Logger.V(1).
			Info("failed to validate license", "license", license.Namespace+"/"+license.Name)
		_ = r.Status().Update(ctx, license)
		return requeueRes, nil
	...
	if license.Spec.Type == licensev1.AccountLicenseType &&
		...
		r.Logger.V(1).
			Info("license is active, skip reconcile", "license", license.Namespace+"/"+license.Name)
		return requeueRes, nil
	}
...
func (r *LicenseReconciler) SetupWithManager(
	mgr ctrl.Manager,
