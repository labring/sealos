...
func (r *DebtReconciler) DetermineCurrentStatus(oweamount int64, userUID uuid.UUID, updateIntervalSeconds int64, lastStatus v1.DebtStatusType) (v1.DebtStatusType, error) {
...
func (r *DebtReconciler) determineCurrentStatusWithSubscription(oweamount int64, userUID uuid.UUID, updateIntervalSeconds int64, lastStatus v1.DebtStatusType) (v1.DebtStatusType, error) {
	...
	if lastStatus == v1.NormalPeriod || lastStatus == v1.LowBalancePeriod || lastStatus == v1.CriticalBalancePeriod {
	...
	if lastStatus == v1.DebtDeletionPeriod && updateIntervalSeconds >= DebtConfig[v1.FinalDeletionPeriod] {
		return v1.FinalDeletionPeriod, nil
	}
	return lastStatus, nil // Maintain current debt state if no transition
}

func determineCurrentStatus(oweamount int64, updateIntervalSeconds int64, lastStatus v1.DebtStatusType) v1.DebtStatusType {
	if oweamount > 0 {
...
func (r *DebtReconciler) setupSmsConfig() error {
	...
	smsClient, err := utils2.CreateSMSClient(os.Getenv(SMSAccessKeyIDEnv), os.Getenv(SMSAccessKeySecretEnv), os.Getenv(SMSEndpointEnv))
	if err != nil {
...
func setDefaultDebtPeriodWaitSecond() {
	DebtConfig[v1.DebtDeletionPeriod] = env.GetInt64EnvWithDefault(string(v1.DebtDeletionPeriod), 7*v1.DaySecond)
	DebtConfig[v1.FinalDeletionPeriod] = env.GetInt64EnvWithDefault(string(v1.FinalDeletionPeriod), 7*v1.DaySecond)
	...
	EmailTemplateZHMap, EmailTemplateENMap = make(map[v1.DebtStatusType]string), make(map[v1.DebtStatusType]string)
