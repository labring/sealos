...
// Credit 表示用户的credits信息
type Credit struct {
	ID           uuid.UUID         `json:"id"`                 // Credit ID
	...
	CreditStatus CreditStatus      `json:"status"`             // 状态：active（有效）, expired（过期）, used_up（已用完）
	Metadata     map[string]string `json:"metadata,omitempty"` // 其他元数据
}

type CreditStatus string
type CreditRecordType string
type CreditRecordReason string

const (
	CreditStatusActive  CreditStatus = "active"
	CreditStatusExpired CreditStatus = "expired"
	CreditStatusUsedUp  CreditStatus = "used_up"

	CreditRecordTypeIssue   CreditRecordType = "issue"
	CreditRecordTypeConsume CreditRecordType = "consume"

	CreditRecordReasonResourceAccountTransaction CreditRecordReason = "AccountTransaction"
)

// CreditTransaction 表示credits的使用或发放记录
type CreditTransaction struct {
	...
	CreditID             uuid.UUID          `json:"credit_id"`                        // 关联的Credit ID
	UsedAmount           int64              `json:"used_amount"`                      // 使用额度
	CreatedAt            time.Time          `json:"created_at"`                       // 操作时间
	Reason               CreditRecordReason `json:"reason"`                           // 操作原因（如"AccountTransaction"）
}
