...
// spec2Status cp spec to status and update status
...
func (r *ImageReconciler) spec2Status(ctx context.Context, img *imagehubv1.Image) error {
	...
	img.Status.Name = img.Spec.Name
	img.Status.DetailInfo = img.Spec.DetailInfo
	return r.Client.Status().Update(ctx, img)
}

// addLable add lable to image.
func (r *ImageReconciler) addLable(ctx context.Context, img *imagehubv1.Image) error {
	if img.Labels == nil {
		img.Labels = map[string]string{}
	}
	img.Labels[imagehubv1.HubSealosOrgLabel] = img.Spec.Name.GetOrg()
	img.Labels[imagehubv1.HubSealosRepoLabel] = img.Spec.Name.GetRepo()
	img.Labels[imagehubv1.HubSealosTagLabel] = img.Spec.Name.GetTag()
	return r.Client.Update(ctx, img)
}
