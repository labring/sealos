...
func downLoadSealos(ssh ssh.Interface, ip string) error {
	cmd := fmt.Sprintf("sudo wget -c https://github.com/labring/sealos/releases/download/v4.1.3/sealos_4.1.3_linux_amd64.tar.gz && sudo tar zxvf sealos_4.1.3_linux_amd64.tar.gz sealos && sudo rm -rf sealos_4.1.3_linux_amd64.tar.gz && sudo chmod +x sealos && sudo mv sealos /usr/bin")
...
func modifySSHConfig(ssh ssh.Interface, ip string) error {
	cmd := fmt.Sprintf("sudo sed -i '63c PasswordAuthentication yes' /etc/ssh/sshd_config && sudo sed -i 's/# StrictHostKeyChecking ask/StrictHostKeyChecking no/' /etc/ssh/ssh_config && sudo systemctl restart sshd")
	if err := ssh.CmdAsync(ip, cmd); err != nil {
		return fmt.Errorf("execute commend: %s,error: %v", cmd, err)
	}
	return nil
