...
BUILD_STAMP ?= $(shell date -u '+%Y-%m-%d %H:%M:%S %Z')
...
.PHONY: local
local: ## 构建二进制
	docker run --rm -v ${PWD}:/go/src/github.com/fanux/sealos -w /go/src/github.com/fanux/sealos golang:1.12-stretch  go build -ldflags "-X 'github.com/fanux/sealos/version.Version=${BUILD_VERSION}' -X 'github.com/fanux/sealos/version.Build=${COMMIT_SHA1}' -X 'github.com/fanux/sealos/version.Buildstamp=${BUILD_STAMP}'"
