...
func GetQuery(query *api.VMRequest) (string, error) {
	...
	switch query.Type {
	case "cpu":
		result = "round(sum(container_cpu_usage_seconds_total{namespace=~\"$namespace\",pod=~\"$pod.*\"}) by (pod) / sum(cluster:namespace:pod_cpu:active:kube_pod_container_resource_limits{namespace=~\"$namespace\",pod=~\"$pod.*\"}) by (pod) /1000,0.01)"
	case "memory":
	...
	case "average_cpu":
		result = "avg(round(sum(container_cpu_usage_seconds_total{namespace=~\"$namespace\",pod=~\"$pod.*\"}) by (pod) / sum(cluster:namespace:pod_cpu:active:kube_pod_container_resource_limits{namespace=~\"$namespace\",pod=~\"$pod.*\"}) by (pod)/1000,0.01))"
	case "average_memory":
