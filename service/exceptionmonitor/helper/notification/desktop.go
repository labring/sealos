...
func CreateNotification(
	...
	notificationMessage, zhNotificationMessage string,
) {
	...
	message := fmt.Sprintf(
		"Because %s , Database %s current status : %s , Please check in time.",
		notificationMessage,
		notificationInfo.DatabaseClusterName,
		notificationInfo.ExceptionStatus,
	)
	zhMessage := fmt.Sprintf(
		"因为 %s , 数据库 %s 当前状态 : %s , 请及时检查.",
		zhNotificationMessage,
		notificationInfo.DatabaseClusterName,
		notificationInfo.ExceptionStatus,
	)
	...
	_, err := api.DynamicClient.Resource(gvr).
		Namespace(notificationInfo.Namespace).
		Create(context.TODO(), notification, metav1.CreateOptions{})
	if err != nil {
