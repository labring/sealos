...
func GetNotificationMessage(notificationInfo *api.Info) string {
	...
	if notificationInfo.NotificationType == ExceptionType &&
		...
		notificationInfo.FeishuInfo = commonElements
		notificationInfo.FeishuInfo = append(notificationInfo.FeishuInfo, exceptionElements...)
...
func createCard(headerTemplate, headerTitle string, elements []map[string]string) map[string]any {
	cardElements := make([]map[string]any, len(elements))
	card := map[string]any{
		"config": map[string]bool{
			"wide_screen_mode": true,
		},
...
func addNonEmptyFieldsToElements(nsQuota *api.NameSpaceQuota, elements *[]map[string]string) {
	fields := map[string]string{
		...
		"ObjectStorageSizeLimit":   "对象存储总量",
		"ObjectStorageSizeUsage":   "对象存储使用率",
		"ObjectStorageBucketLimit": "对象存储bucket总量",
		"ObjectStorageBucketUsage": "对象存储bucket使用率",
		"PodsLimit":                "pods总量",
		"PodsUsage":                "pods使用率",
	...
	quotaValues := map[string]string{
		...
		"ObjectStorageSizeLimit":   nsQuota.ObjectStorageSizeLimit,
		"ObjectStorageSizeUsage":   nsQuota.ObjectStorageSizeUsage,
		"ObjectStorageBucketLimit": nsQuota.ObjectStorageBucketLimit,
		"ObjectStorageBucketUsage": nsQuota.ObjectStorageBucketUsage,
		"PodsLimit":                nsQuota.PodsLimit,
		"PodsUsage":                nsQuota.PodsUsage,
