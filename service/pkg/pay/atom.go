...
type StripeServiceInterface interface {
	CreateWorkspaceSubscriptionSession(
		req PaymentRequest,
		workspace, regionDomain, planName, period string,
...
func (s *AtomPaymentService) GetPayment(
	...
	if !ok {
		return nil, fmt.Errorf("unexpected query response type: %T", execute)
	}
	return response, nil
}
...
func (s *AtomPaymentService) createPaymentWithMethod(
	...
	response, ok := execute.(*responsePay.AlipayPayResponse)
	if !ok {
		return nil, fmt.Errorf("unexpected payment response type: %T", execute)
	}
	return response, nil
}
...
func (s *AtomPaymentService) CancelPayment(
	...
	if !ok {
		return nil, fmt.Errorf("unexpected cancel response type: %T", execute)
	}
	return response, nil
}
