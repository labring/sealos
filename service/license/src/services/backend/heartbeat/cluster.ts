...
export async function ClusterHeartbeat(payload: ClusterHeartbeatPayload) {
    ...
    const record = genRecord(payload);
    ...
    return await collection.findOneAndReplace({clusterID: record.clusterID}, record, {upsert: true});
}

function genRecord(payload: ClusterHeartbeatPayload) {
    const now = new Date();
